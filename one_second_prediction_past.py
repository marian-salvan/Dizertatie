...
def plot_predictions(real, predicted, column):
    ...
    ax.legend()
    plt.savefig('./images/one_second_data_past/new_batch_' + column + '.png')
    plt.close(fig)
...
def plot_learning_rates(hist, column, score):
    ...
    ax.set_ylabel('Score')
    ax.legend()
    plt.savefig('./images/one_second_data_past/new_batch_' + score + '_' + column + '.png')
    plt.close(fig)


for column in OUTPUTS:

    df = pd.read_csv("E:/data/one_second_data/" + column + ".csv")

    X = df[df.columns[0:9]].values      # predictor feature columns (10 X m)
    y = df[df.columns[9:10]].values  # predicted class column (1 X m)
    split_test_size = 0.1
    ...
    # save model to png
    plot_model(model, to_file="./images/new_batch_one_second_prediction_past.png", show_shapes=True, show_layer_names=True)
    ...
    file = open("results/one_second_data_past/new_batch_result.txt", "a")
