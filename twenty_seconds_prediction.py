...
OUTPUTS = [ #'inlet_probe_1_rack_1', 'inlet_probe_2_rack_1', 'inlet_probe_3_rack_1', 'inlet_probe_4_rack_1',
           ...
           #'outlet_probe_1_rack_1',
            'outlet_probe_1_rack_2', 'outlet_probe_1_room',
...
def plot_predictions(real, predicted, column):
    fig, ax = plt.subplots()
    line1, = ax.plot(np.arange(20), real)
    line2, = ax.plot(np.arange(20), predicted)
...
for column in OUTPUTS:
    ...
    df = pd.read_csv("E:/data/twenty_seconds_data/" + column + ".csv")
    print(df.shape)

    X = df[df.columns[0:45]].values      # predictor feature columns (10 X m)
    y = df[df.columns[45:65]].values  # predicted class column (1 X m)
    split_test_size = 0.2
    ...
    file = open("./results/twenty_seconds_data/result_no_dp.txt", "a")
